...
<div id="file-tree" class="ma-0 pa-0">
  <v-treeview dense open-all :items="state.files" :open.sync="state.filesOpen" class="subtitle ma-0 pa-0">
    <template v-slot:prepend="{ item, open }">
      <v-icon v-if="item.type === 'directory'" small color="amber lighten-1">
        {{ open ? 'mdi-folder-open' : 'mdi-folder' }}
      </v-icon>
      <v-icon v-else small :color="fileIcon(item).color">
        {{ fileIcon(item).icon }}
      </v-icon>
    </template>
