...
function SMTP (messenger) {
  ...
  const onRcptTo = (address, session, callback) => {
    if (!address.address.endsWith(messenger.config.messenger.smtp.domain)) {
  ...
  this.start = (callback) => {
    this.server = new SMTPServer({
      name: messenger.config.messenger.smtp.domain,
      authOptional: true,
    ...
    this.securePort = messenger.config.messenger.smtp.securePort;
