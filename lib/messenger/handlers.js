...
function Handlers (messenger) {
  ...
  this.sendAndDelete = (envelope, id) => {
    messenger.smtp.outgoing.send(envelope, (error, info) => {
      if (error) {
        ...
        messenger.database.messages.deleteOne({ id }, (error) => {
          if (error) {
  ...
  this.signup = (data) => {
    const envelope = this.envelope(this.templates.signup, data);
    this.sendAndDelete(envelope, data.id);
  };

  //////////

  this.reset = (data) => {
    const envelope = this.envelope(this.templates.passwordReset, data);
    this.sendAndDelete(envelope, data.id);
  };
  ...
  messenger.events.on('mongo:interrogative.messages:insert', (event) => {
    ...
    } else if (event.data.object === 'reset') {
