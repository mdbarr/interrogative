...
function InteractionAPI (container) {
  ...
  container.interaction.get('/api/who', (req, res, next) => {
    ...
    const response = items.map((item) => `${ item.name.padEnd(nameLength + 12, ' ') }` +
        `${ moment(item.signin).format('YYYY-MM-DD HH:mm') } (${ item.address })`).join('\n');
  ...
  this.start = (callback) => container.interaction.listen(container.config.interaction.port,
