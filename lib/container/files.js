...
function Files (container, options = {}) {
  ...
  const exclusions = this.config.exclusions.join('|').replace(/[.]/g, '\\.');
  ...
  this.open = (path) => {
    ...
    return fs.stat(path, (error, stat) => {
      ...
      console.log('stat', stat.size, container.config.container.maxOpenSize);

      if (stat.size > container.config.container.maxOpenSize) {
        return container.events.emit({
