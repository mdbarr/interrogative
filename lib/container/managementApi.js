...
function ManagementAPI (container) {
  ...
  container.api.get('/ws/:id/shell', (req, res, next) => {
    ...
    console.log('[terminal]', instance, cols, rows);

    if (terminals.has(instance)) {
      console.log('[terminal] attaching to existing instance', instance);
      const terminal = terminals.get(instance);
      terminal.add(shed);

      return terminal;
    }

    console.log('[terminal] creating new instance', instance);
