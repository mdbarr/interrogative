...
function Persistence (container) {
  ...
  this.save = (callback) => {
    ...
    if (this.changed) {
      ...
      return async.each(this.persist, this.write, () => {
  ...
  container.events.on('file:upload:success', () => { this.changed = true; });
  container.events.on('files:file:closed', () => { this.changed = true; });
  container.events.on('files:file:opened', () => { this.changed = true; });
  container.events.on('messages:message:send', () => { this.changed = true; });
  container.events.on('timeline:events:list', () => { this.changed = true; });
