...
function Persistence (container) {
  this.files = {
    open: join(container.config.container.persistence, '/open.json'),
    uploads: join(container.config.container.persistence, '/uploads.json'),
    timeline: join(container.config.container.persistence, '/timeline.json')
  };
  ...
  this.start = (callback) => {
    fs.mkdir(container.config.container.persistence, { recursive: true }, (error) => {
