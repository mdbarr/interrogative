...
function Interviews (manager) {
  ...
  const scopes = [ { name: 'Current Editor Tab' }, { name: 'Global' } ];

  const filters = [ { name: 'File Extension' }, { name: 'File Name' }, { name: 'File Type' } ];

  this.models.action = ({
    scope = scopes[0], filter = filters[0], icon, hint, action
  }) => {
    const model = {
      scope,
      filter,
      ...
      if (user.id === id) {
        if (user.role === 'candidate') {
          ...
          if (now >= interview.start && now <= interview.stop) {
            ...
            return 'Thus interview is no longer available.';
