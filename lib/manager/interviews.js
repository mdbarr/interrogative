...
function Interviews (manager) {
  ...
  this.models.user = (interview, {
    ...
    const invitation = role === 'interviewer' ? 'now' :
      interview.start - milliseconds(interview.invitation);
    ...
    const model = {
      ...
      email,
      role,
      invitation: sendEmails ? invitation : false,
  ...
  this.models.interview = ({
    ...
    manager.utils.hidden(model, '$queueMessages', (callback) => {
      const base = {
        candidate: null,
        interview: null,
        when: moment(this.start).calendar(),
      ...
      for (const user of this.users) {
        if (user.invitation !== false) {
          const message = Object.assign({}, base, {
            id: `${ user.id }/i`,
            object: `invitation-${ user.role }`,
            interview: this.id,
            email: user.email,
      ...
      return async.each(messages, (message, next) => {
        return manager.database.messages.updateOne({ id: message.id },
          { $set: message },
          { upsert: true },
          next);
      }, callback);
