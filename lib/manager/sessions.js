...
function Sessions (manager) {
  ...
  this.login = (req, res, next) => {
    ...
    return manager.users.lookup(username, (error, user) => {
      if (error || !user || !user.verified || user.deleted) {
        res.clearCookie(manager.config.sessions.cookie);
        return next(new errors.UnauthorizedError('Invalid username or password'));
      }

      if (!user.verified) {
        return next(new errors.BadRequestError('Account not activated'));
      }
  ...
  this.sanitize.properties = [ 'password', 'mfa', 'attributes' ];
  ...
  this.administrator = (req, res, next) => this.validate(req, res, (error) => {
    if (error) {
      return next(error);
    }
