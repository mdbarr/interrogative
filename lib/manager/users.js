...
function Users (manager) {
  this.model = ({
    ...
    manager.utils.hidden(model, '$save', (callback) => model.$password(() => manager.database.users.updateOne({ id: model.id },
      { $set: model },
      { upsert: true },
      callback)));
  ...
  this.validatePassword = function(user, input, callback) {
    ...
    return argon2.verify(user.password, input).
      then((verified) => callback(null, verified)).
      catch((error) => callback(error));
  };
  ...
  this.email = (email, callback) => manager.database.users.find({ email }).count((error, count) => {
    if (error) {
      return callback(error);
    } else if (count) {
      return callback(null, false);
  ...
  this.load = (callback) => manager.database.users.findOne({ administrator: true }, (error, admin) => {
    if (error || !admin) {
      const password = process.env.INTERROGATIVE_ADMIN_PASSWORD ||
      ...
      return this.hashPassword(password, (error, hash) => {
        ...
        this.administrator = this.model({
          name: 'Administrator',
          email: process.env.INTERROGATIVE_ADMIN_EMAIL || 'administrator@interrogative.io',
          password: hash,
  ...
  this.activate = (req, res, next) => this.lookup({ 'attributes.signup.id': req.params.id }, (error, user) => {
    if (error || !user) {
    ...
    return user.$activate(() => manager.interviews.sandbox(user, () => {
      req.authorization = {
  ...
  this.forgot = (req, res, next) => this.lookup(req.body.email, (error, user) => {
    if (error || !user) {
  ...
  this.reset = (req, res, next) => this.lookup({ 'attributes.reset.id': req.params.id }, (error, user) => {
