...
function Websocket (manager) {
  ...
  this.connect = ({
    source, address, port, path,
  }, callback) => {
    console.log(`[ws] connecting to ${ address }:${ port } from ${ source.realIP }`);
  ...
  this.attach = (req, res, next) => manager.interviews.lookup(req.params.id, (error, interview) => {
    if (error || !interview) {
    ...
    return this.connect({
      source: req,
      address: interview.state.container.address,
      port: manager.config.container.port,
      path,
    }, (error, proxy) => {
      if (error) {
